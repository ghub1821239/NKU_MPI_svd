...
我在后面开始写 profiling 的时候，最先关心的其实不是“表格怎么画”，而是“到底应该统计哪些时间”。因为这次实验里有主进程、有子进程、有点对点通信、有广播同步，还有一些看起来很像时间、但含义其实不一样的量。如果这里口径没定好，后面所有分析都会很乱。

当时我主要围绕下面几个问题和 AI 讨论：
\begin{enumerate}
    \item 我们最后到底应该看主进程的时间，还是看所有进程的总时间。
    \item 如果想说明程序真正有没有变快，关键路径应该怎么统计。
    \item MPI 里有哪些现成 API 可以用来做时间统计。
    \item 哪些量适合拿来说明“用户实际等了多久”，哪些量适合拿来说明“总资源消耗有多大”。
    \item 两个 MPI 版本的 profiling 口径不完全一样时，后面该怎么对齐分析。
\end{enumerate}

\subsection{2. 最后定下来的时间口径}

最后我没有把所有时间都混在一起，而是把它们拆成了几层来看。

\begin{enumerate}
    \item 第一层是关键路径时间。这里我主要看 \texttt{time gkh iteration(ms)} 和 \texttt{MPI\_MAX wall}。因为这两个量更接近“这轮程序真正跑完，用户等了多久”。
    \item 第二层是主进程本地工作量。这里我重点看 \texttt{root process block} 和 \texttt{root extract+insert}，因为它们能直接反映 root 自己是不是已经很重了。
    \item 第三层是通信和同步负担。这里我看的是 \texttt{comm subtotal}，也就是把所有进程上的通信时间累计起来之后得到的值。它不是关键路径时间，但可以说明总体通信负担是不是在变重。
    \item 第四层是本地更新量。这里看的是 \texttt{local update}，也就是各个进程在本地应用旋转、更新矩阵的累计时间。
    \item 第五层是调度器的 \texttt{Time Use}。这个量不代表关键路径，但它能说明整次作业到底吃掉了多少 CPU 资源，所以特别适合用来说明“资源用得更多了，但程序不一定真的更快”。
\end{enumerate}

也就是说，我最后并不是只看一个总时间，而是把“关键路径”“主进程负担”“通信负担”“本地更新”“总资源消耗”这几层分开来看。这样做的好处是，即使程序没有加速，我们也能知道它到底是卡在 root、卡在通信，还是卡在任务供给上。

\subsection{3. 为什么最后选择以关键路径为主}

我后面和 AI 讨论下来，最重要的一个收获其实就是：\textbf{并行程序分析时，不能把所有时间都按同一个层次去理解。}

比如一开始看到 \texttt{comm subtotal} 比 \texttt{time gkh iteration(ms)} 还大时，我也会觉得这个数是不是有问题。后来才慢慢理清楚，这两个量本身就不是一个层次：
\begin{enumerate}
    \item \texttt{time gkh iteration(ms)} 和 \texttt{MPI\_MAX wall} 反映的是关键路径，也就是用户真正等了多久。
    \item \texttt{comm subtotal} 和 \texttt{local update} 是把多个进程上的时间做累计之后得到的，所以它们本来就可以比关键路径更大。
\end{enumerate}

把这件事想清楚之后，我后面的表头和解释也跟着改了。也就是后来报告里写的“关键路径Wall”“通信累计”“更新累计”这些说法，本质上就是在避免把不同层次的时间混为一谈。

\subsection{4. 这次实际用到的 MPI 时间统计思路}

在实现上，我这次没有去用特别复杂的外部 profiler，而是优先在代码里加时间点，把程序运行过程拆开来统计。这里最常用的就是 \texttt{MPI\_Wtime()}，因为它比较适合拿来做 MPI 程序内部的分段计时。

具体来说，思路就是：
\begin{enumerate}
    \item 在一段逻辑开始前记一个时间点；
    \item 在结束后再记一个时间点；
    \item 用前后差值算这一段花了多少时间；
    \item 最后再根据这段时间属于“root 处理”“通信”“本地更新”中的哪一类，把它累计到对应统计量里。
\end{enumerate}

像 root 处理块、收发控制消息、广播旋转、应用旋转、最终 gather 这些时间，后面基本都是按这个思路一点一点加进去的。

\subsection{5. AI 在这一部分真正帮到我的地方}

如果只说“AI 帮我写了点文字”，那其实太表面了。对我来说，它在这一部分真正帮到我的地方主要有三个：
\begin{enumerate}
    \item 帮我把“应该看哪个时间”这件事先分层理清楚了，特别是关键路径时间和累计时间不能混着理解这一点。
    \item 帮我把原来代码里已经有的一些统计项重新组织成可以写进报告的口径，而不是只停留在打印了一堆数字。
    \item 帮我把后面表格里的解释改得更严谨一些，避免出现“这些时间看起来加不起来，所以是不是数据错了”这种误读。
\end{enumerate}
