...
\textbf{问：如果想说明程序真正有没有变快，关键路径（rank=0）应该怎么统计？}

\textbf{答：} 如果只是想回答“这个程序最后到底有没有变快”，那最应该优先看的不是所有进程加起来花了多少时间，而是关键路径上到底花了多少时间。因为对用户来说，真正关心的是这一轮程序什么时候结束，而不是后台一共累计消耗了多少时间。
...
\textbf{问：MPI 里有哪些现成 API 可以用来做时间统计？}

\textbf{答：} 这次最直接、最好用的其实就是 \texttt{MPI\_Wtime()}。它适合做 MPI 程序内部的分段计时，比如：
\begin{enumerate}
...
\textbf{问：哪些量适合拿来说明“用户实际等了多久”，哪些量适合拿来说明“总资源消耗有多大”？}

\textbf{答：} 这里最容易混掉，所以后来我和 AI 反复确认过，最后把这些量分成了两层。
