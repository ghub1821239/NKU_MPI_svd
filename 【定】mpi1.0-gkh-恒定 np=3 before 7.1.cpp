...
bool gkh_svd_from_bidiagonal(Matrix &U, Matrix &B, Matrix &V, int max_iter, double tol)
{
    ...
    if (size != 3)
    {
        if (rank == 0)
        {
            std::cerr << "[gkh-mpi] fixed-role prototype only supports size == 3\n";
        }
        return false;
    }
    ...
    const int TAG_U_PRE = 200;
    const int TAG_V_PRE = 220;
    const int TAG_TASK_COUNT = 240;
    const int TAG_U_STEP = 300;
    const int TAG_V_STEP = 320;
    ...
    if (rank == 0)
    {
        ...
        for (int iter = 0; iter < max_iter; ++iter)
        {
            ...
            send_row_rotations_to_rank(u_ops, 1, TAG_U_PRE, MPI_COMM_WORLD);
            send_row_rotations_to_rank(v_ops, 2, TAG_V_PRE, MPI_COMM_WORLD);
            MPI_Send(&task_count, 1, MPI_INT, 1, TAG_TASK_COUNT, MPI_COMM_WORLD);
            MPI_Send(&task_count, 1, MPI_INT, 2, TAG_TASK_COUNT, MPI_COMM_WORLD);
            ...
            for (int t = 0; t < task_count; ++t)
            {
                ...
                send_row_rotations_to_rank(u_step_ops, 1, TAG_U_STEP, MPI_COMM_WORLD);
                send_row_rotations_to_rank(v_step_ops, 2, TAG_V_STEP, MPI_COMM_WORLD);
                --idx;
            }
        }
    ...
    else if (rank == 1)
    {
        ...
        for (int iter = 0; iter < max_iter; ++iter)
        {
            std::vector<RowRotation> u_ops = recv_row_rotations_from_rank(0, TAG_U_PRE, MPI_COMM_WORLD);
            apply_row_rotations_local(Ut, u_ops);

            int task_count = 0;
            MPI_Recv(&task_count, 1, MPI_INT, 0, TAG_TASK_COUNT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if (task_count == 0)
            {
                break;
            }
            ...
            for (int t = 0; t < task_count; ++t)
            {
                std::vector<RowRotation> u_step_ops = recv_row_rotations_from_rank(0, TAG_U_STEP, MPI_COMM_WORLD);
                apply_row_rotations_local(Ut, u_step_ops);
            }
        }
    ...
    else if (rank == 2)
    {
        ...
        for (int iter = 0; iter < max_iter; ++iter)
        {
            std::vector<RowRotation> v_ops = recv_row_rotations_from_rank(0, TAG_V_PRE, MPI_COMM_WORLD);
            ...
            for (int t = 0; t < task_count; ++t)
            {
                std::vector<RowRotation> v_step_ops = recv_row_rotations_from_rank(0, TAG_V_STEP, MPI_COMM_WORLD);
                apply_row_rotations_local(Vt, v_step_ops);
            }
        }
